...
export const generateRPM = async (data: RPMInput): Promise<string> => {
  try {
    ...
    // Force remove any inline text color styles to prevent invisible (white) text on a white background.
    // This is a safeguard in case the model ignores the prompt instruction.
